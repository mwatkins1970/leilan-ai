...
## Chamber audio bleeding into a later, unrelated page (fixed 2026-07-20)

**Symptom** (reported by M, 2026-07-20, live on leilan.ai): starting in OVS Chapel, M went through the door into the main chamber, then through another door into a third chamber. During that second immersive dive (the new chamber rising), OVS Chapel's ambience was audibly superimposed over the immersive page's own rising-tone soundscape — i.e. audio from a chamber page *two full navigations back* (`prism/ovs-chapel` → `/immersive` → `prism/main` → `/immersive`) was still sounding on a page that has entirely separate, unrelated audio code.

**Root cause (best-supported explanation, not directly observed)**: every door in every chamber funnels through a single call site — `enterArchway(wallNum)` (`prism.js` line 1853) — which calls `fadeOutDrone(2.0)` immediately, with the actual `AudioContext.close()` landing ~2.05s later, comfortably inside the 3.2s archway-zoom before `window.location.href` fires (see *Archway Exit Audio* above). That path is sound on its own and was confirmed to be the only call site. But it is the *only* teardown path — anything that hides or discards a chamber page **without** going through that specific click never runs it: browser back/forward navigation, or a bfcache freeze that doesn't fully silence a `setTimeout`-driven event scheduler (this codebase's `tick()` scheduler, `AUDIO.md` Part 2). Chrome aggressively bfcaches eligible pages by default on essentially every cross-document navigation, not just deliberate back-button use, so a prior chamber visit can sit frozen-but-alive in the tab's history stack for a long time; if audio isn't cleanly stopped at that freeze point, it can keep generating output that mixes with whatever page is later on-screen. This fits the reported symptom precisely and is a well-documented category of real-world bfcache + Web Audio bugs.

**Fix**: a defensive `pagehide` listener added to both audio systems — `public/scripts/prism.js` (right before `enterArchway`) and `src/pages/immersive.astro` (right after `fadeOutImmersiveSoundscape`). `pagehide` fires in every case `unload` does, *plus* bfcache freezes, and — unlike `beforeunload`/`unload` — does not itself disqualify the page from bfcache eligibility (per the Page Lifecycle API / web.dev's bfcache guidance). Each handler immediately clears any pending event-scheduler timers and hard-closes that page's `AudioContext` if it isn't already closed, with no fade (the page is already gone by the time anyone would hear one). This closes the gap regardless of *how* a page ends up hidden — back/forward navigation, bfcache, or anything else — rather than only covering the one deliberate in-app click path.

**Caveat**: this was diagnosed from the code (the archway path is the only teardown trigger; bfcache is the most plausible way to bypass it) rather than reproduced directly — M's report was on the live site, not something reproducible in this session. If a similar bleed is ever reported again post-fix, first rule out a simple leftover browser tab (the event engine loops indefinitely with no auto-timeout as long as a chamber tab stays open — completely separate from this bug) before assuming the `pagehide` handler failed.
