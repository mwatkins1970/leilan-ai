...
**Walls 3, 4, 6: navigation verified 2026-08-03, geometry still unverified.** Each of `?from=3|4|6` (plus `from=2` as a control) was driven through a full dive headlessly: all four reach the correct destination chamber with **no console or page errors**. What that does *not* prove is the **visual** side — that the second prism rises at the correct adjacent position for each — since that needs an eye on the animation, not a URL check. So: not broken, but not signed off either.
...
- `src/data/fieldNotes.ts` — data model + every note's metadata: `title`/`titleHtml`, `shortTitle`/`shortTitleHtml`, `description`/`descriptionHtml`, `dek`/`dekHtml`, `kicker`, `sanctuaryUrl`, `related`, `topNote`. The `*Html` variants are **display-only** (italics/markup) so the plain fields stay clean for `<title>`/meta/JSON-LD. Three further optional fields exist for SEO tuning (added 2026-08-03): **`metaTitle`** overrides the whole `<title>`/`og:title` while leaving the visible H1 alone (default remains `` `${title} | Leilan.ai Field Notes` ``); **`indexAnchor`** gives a note descriptive link text on `/field-notes/` only, keeping `shortTitle` short for the breadcrumb and its JSON-LD; **`endNote`** is HTML appended to the body by the field-note route only.
  > **`endNote` exists for a reason — use it, don't edit the wall text.** Wall-derived notes render the *same* HTML file as the chamber popups, so a contextual cross-link written into `src/data/wall-texts/*.html` would also appear inside the sanctuary. No wall text contains a `/field-notes/` link, and that separation is deliberate (in-chamber "open field-note version" links are an unbuilt *optional* on the launch checklist, not a thing to introduce by accident). `endNote` carries such links on `petertodd` and `archaeology`.
...
- ✅ **On-page pass for the Leilan-goddess queries** (2026-08-03, deployed) — targeted at "Leilan AI", "Leilan AI goddess", "Leilan goddess", "Matthew Watkins Leilan", "SolidGoldMagikarp Leilan". New `metaTitle` on `what-is-leilan` + `solidgoldmagikarp`; rewritten `what-is-leilan` meta description; a Tell Leilan / *Puzzle & Dragons* disambiguation paragraph near the top of `what-is-leilan`; the "reasearch" typo fixed on the book page; contextual links to `what-is-leilan` from `petertodd` and `archaeology` (via `endNote`) — `glitch-tokens` and `solidgoldmagikarp` already had them; a descriptive "Start here" anchor on the index; and a crawlable "What is Leilan?" link in the homepage sidebar nav. Canonicals/robots/sitemap/trailing-slash/redirect policy untouched. **M's follow-up:** request re-indexing in Search Console for `/field-notes/what-is-leilan/` and `/field-notes/solidgoldmagikarp/`.
- ✅ **Canonicalisation confirmed live (2026-08-03)** — `http://leilan.ai/` → 301 → `https://leilan.ai/`; `http://www.leilan.ai/` → 301 → `https://www.leilan.ai/` → 301 → `https://leilan.ai/`. Netlify is handling both; nothing to configure.
- **Still open:** Bing Webmaster Tools verify + sitemap submit; seed backlinks (publisher / Substack / GitHub / Zenodo / Hugging Face / Archive.org) with varied anchor text; optional polish — in-chamber "Open field-note version" links (sanctuary→cloister). *(The purpose-made 1200×630 OG card shipped 2026-07-14 — see SEO mechanics above.)*
...
- **Moonlight** (2026-08-07) — the real lunar phase now *lights* the chamber, not just the moon sprite. `updateMoonlight()` in `prism.js` turns `moonAge01()` into `--moonlight` (0 new → 1 full, shaped by `MOONLIGHT_GAMMA = 2.2` so only the nights around full read as lit); night-only, day mode is 0. **Two expressions, because one alone doesn't carry every chamber**: (1) a cool additive wash on `.wall-panel::before` (a second background layer above the existing overhead gloss; peak alpha `--moonlight-max`, both tokens on `:root` in `prism.css`) — measured to lift the Research Lab moiré by 11/255 at the wall top but to do *nothing* in the Central Chamber or OVS Chapel, whose art already sits at 210–240 luminance; (2) **moonglow** — the broad soft dome of sky-light around a bright moon (`MOON_GLOW_R` = 11 moon-radii, peak alpha `MOON_GLOW_PEAK`, baked into the cached `moonSprite` so it costs one blit); this is the component that makes the feature *visible* — measured at +14.7/255 mean luminance across the sky band at full moon; (3) **star washing** — each star carries a precomputed `moonWashK` (0.90 for the 5–9px faint ones down to 0.25 for the 12–24px bright ones), so a full moon drowns the faint stars and leaves the bright, which works in every chamber because the sky is shared. Strictly additive on the art: `--moonlight: 0` reproduces the chambers exactly as signed off 2026-08-03. Zero per-frame cost (one CSS property set at load + on the day/night toggle). The ASCII gallery is unaffected without special-casing — it replaces `.wall-panel::before` wholesale. **Preview any phase**: `?moon=0.5` (full) / `?moon=0` (new) on any chamber URL — flip between two tabs, which is the only honest way to judge a lighting change — or `localStorage.moonPhase = '0.5'`, which survives navigation between chambers. Both drive the drawn moon *and* the lighting so they always agree; `delete localStorage.moonPhase` to return to the real sky. **Gotcha**: `prism.js` line ~56 wipes `location.search` as soon as `?wall=` is consumed, so any new URL param must be read up there with the others (`_urlMoonPhase`) — reading `location.search` later silently returns empty.
...
### Resolved
- **Word-panel click routing (was Known Issue #2 — retired 2026-08-03, no fix needed)**: the entry warned that the Chrome `preserve-3d` hit-test workaround "may still have issues in Research Lab / OVS Chapel word panels — test before assuming it works". Tested exhaustively (Playwright): every one of the six Research Lab walls, and all five OVS Chapel word walls (wall 4 is the star wall and correctly has no word), opens the *correct* wall's text frame at *every* rotation position — no misrouting to the back wall, no errors. The capture-phase `wallFromScreenX()` zone router plus `hitFrameControl()` is doing its job. Retired rather than fixed. (The underlying Chrome bug is still real — keep calling business logic directly rather than dispatching synthetic clicks into the 3D container.)
- **Dropped anchor on the UFO wall LW link (fixed 2026-07-31)**: the `mythopoeic-archive-ufo.html` link to *petertodd's Last Stand* had lost its original `#The___petertodd____Leilan__connection` anchor when that content moved to the `/lesswrong-petertodd-last-stand` interstitial. Since that interstitial only ever has the one inbound link (from this wall), the anchor is hardcoded directly onto its outbound `lwUrl` rather than plumbed through as a generic param.
- **Two other "Known Issues" turned out to already be fixed (found 2026-07-31, not actually bugs)**: the book-subtitle line item claimed `ovs-chapel-hyperstition.html` had the wrong subtitle — checked the file directly, it already reads *A Descent Into the AI Underworld* correctly. The "become become" typo line item claimed all three LW interstitials had a duplicated word — checked all three, none do. Both entries removed; whatever fixed them wasn't logged at the time.
...
> **2026-08-22 — credits restored.** M upgraded the Netlify plan for one month to cover the book launch (publication 2026-08-27), replacing the 2026-08-07 block on production deploys. Normal deploy rules apply again: `git push origin main` **only when M explicitly asks**. Revisit around 2026-09-22, when the upgrade lapses and the Free-tier 300-credit cap (and its no-deploy risk) returns.
...
- Recent work history (much now committed; see commit log above):
  ...
  - ~~**Note**: a 642 MB `leilan-ai-2026-05-27.zip` backup sits in the repo root (gitignored). Heaviest live assets pending a launch-time compression pass: `wall-border.png` (5.6 MB), `wall-bg.jpg` (2.5 MB).~~ **Stale — checked 2026-08-03**: the zip is gone from the repo root, and `wall-border.png` / `wall-bg.jpg` were both deleted in the Phase A/B bandwidth pass. `public/images` is 82 MB. Nothing outstanding here.
