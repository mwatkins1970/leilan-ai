...
> **⛔ 2026-08-07 — M has exhausted this cycle's Netlify credits.** No production deploys until the cycle resets (~2026-08-29). Until then `git push origin main` is off the table: do not offer it, do not run it. `git push origin main:wip` (0 credits) is the only push. Preview everything on the Cloudflare tunnel. Delete this note once the cycle has reset and M has deployed again.
